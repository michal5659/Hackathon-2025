...
class IDITAPIClient:
    ...
    async def execute_action(
        ...
        credentials = f"{'Administrator':1111}"
        ...
        try:
            print(f"Calling IDIT API: {method} {endpoint}")
            ...
            async with httpx.AsyncClient() as client:
                ...
                print(result)
                print(f"IDIT API call completed: {result['success']} (took {execution_time:.2f}s)")
                return result

        except httpx.TimeoutException as e:
            print(f"IDIT API timeout: {str(e)}")
            return self._create_error_response("Request timeout", 408, time.time() - start_time)
        except httpx.RequestError as e:
            print(f"IDIT API request error: {str(e)}")
            return self._create_error_response(str(e), 500, time.time() - start_time)
        except Exception as e:
            print(f"IDIT API unexpected error: {str(e)}")
            return self._create_error_response(str(e), 500, time.time() - start_time)
    ...
    def _parse_response(self, response: httpx.Response, execution_time: float) -> Dict[str, Any]:
        ...
        return {
            ...
            "message": response_data.get('message',
                                         response.reason_phrase) if success else f"Error: {response.reason_phrase}",
    ...
    async def health_check(self) -> bool:
        ...
        try:
            ...
            print(f"IDIT API health check failed: {str(e)}")
            return False

    async def submit_policy_inquiry(self, customer_id: str, policy_number: str, inquiry_details: Dict[str, Any]) -> \
            Dict[str, Any]:
